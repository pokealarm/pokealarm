...
def parse_filters_file(mgr, filename):
    try:
        ...
        log.error(
            "PokeAlarm was unable to find a filters file "
...
def parse_rules_file(manager, filename):
    ...
    try:
        ...
        log.error(
            "Encountered error while loading Rules:"
            " {}: {}".format(type(e).__name__, e)
        )
        log.error(
            "PokeAlarm has encountered a 'ValueError' while loading the "
    ...
    try:
        ...
        for key in rules:
            raise ValueError(
                "Unknown Event type '{}'. Rules must be defined "
...
def load_rules_section(set_rule, rules):
    for name, settings in rules.items():
        if "filters" not in settings:
            raise ValueError("{} rule is missing a `filters` section.".format(name))
        if "alarms" not in settings:
            raise ValueError("{} rule is missing an `alarms` section.".format(name))
