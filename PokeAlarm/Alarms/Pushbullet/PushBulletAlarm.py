...
class PushbulletAlarm(Alarm):
    ...
    def startup_message(self):
        if self.__startup_message:
            args = {
                ...
                "message": (
                    "PokeAlarm has successully started!"
                    if self.__startup_text == ""
                    else self.__startup_text
                ),
            }
            try_sending(self._log, self.connect, "PushBullet", self.push_note, args)
    ...
    def send_alert(self, alert, info):
        ...
        try_sending(self._log, self.connect, "PushBullet", self.push_link, args)
    ...
    def get_sender(self, channel_tag):
        req_channel = next(
            (channel for channel in self.__channels if channel["tag"] == channel_tag),
            None,
        )
