...
def parse_filters_file(mgr, filename):
    try:
        ...
        if type(filters_file) is not OrderedDict:
            log.critical(
                'Filters files must be a JSON object: { "monsters":{...},... }'
            )
...
def parse_filter_section(section):
    ...
    for name, settings in section.pop("filters", {}).items():
        ...
        local_dts = dict(
            itertools.chain(default_dts.items(), settings.pop("custom_dts", {}).items())
        )
        if len(local_dts) > 0:
...
def parse_alarms_file(manager, filename):
    try:
        ...
        if type(alarm_settings) is not OrderedDict:
            log.critical(
                "Alarms file must be an object of Alarms objects - { 'alarm1': {...}, ... 'alarm5': {...} }"
            )
            sys.exit(1)
    except ValueError as e:
    ...
    try:
        for name, alarm in alarm_settings.items():
            active = parse_boolean(
                require_and_remove_key("active", alarm, "Alarm objects in file.")
            )
            if active:
...
def parse_rules_file(manager, filename):
    ...
    try:
        ...
        load_rules_section(manager.add_grunt_rule, rules.pop("invasions", {}))

        for key in rules:
            raise ValueError(
                f"Unknown Event type '{key}'. Rules must be defined "
                "under the correct event type. See "
                "example in rules.json.example."
            )
...
def load_rules_section(set_rule, rules):
    for name, settings in rules.items():
        if "filters" not in settings:
            raise ValueError(f"{name} rule is missing a 'filters' section.")
        if "alarms" not in settings:
            raise ValueError(f"{name} rule is missing an 'alarms' section.")
        ...
        if len(settings) > 0:
            raise ValueError(f"Rule {name} has unknown parameters: {settings}")
