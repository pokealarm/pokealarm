    ...
    def handle(self, msg):
        ...
        if words[0].lower() == "remove":
            if words[1].lower() == "raid":
                ...
                self.bot.sendMessage(chat_id, 'Type of alert that does not exist')
            ...
            if words[1].lower() == "raid":
                ...
                if words[2].title() in filters["raids"]["filters"]["filter-name"]["monsters"]:
                    ...
                    self.saveFilters(filters)
                    self.bot.sendMessage(chat_id, 'Raid successfully registered.')

            elif words[1].lower() == "pokemon" or words[1].lower() == "pokémon":
                filters = self.loadFilters()

                if words[2].title() in filters["raids"]["filters"]["filter-name"]["monsters"]:
                    self.bot.sendMessage(chat_id, 'Pokémon already registered.')
                else:
                    filters["monsters"]["filters"]["filter-name"]["monsters"].append(
                        words[2].title())

                    self.saveFilters(filters)
                    self.bot.sendMessage(chat_id, 'Pokémon successfully registered.')
            else:
                self.bot.sendMessage(chat_id, 'Type of alert that does not exist.')
        else:
